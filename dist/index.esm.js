 ...
 */
function encodeTime(now, separator = '_', separateTime = false) {
    if (isNaN(now)) {
 ...
 */
function monotonicFactory(currPrng, separator = '_', separateTime = false) {
    if (!currPrng) {
        currPrng = detectPrng();
    }
    let lastTime = 0;
...
export { detectPrng, encodeRandom, encodeTime, incrementBase32, maxRandomCharacter, monotonicFactory, randomChar, replaceCharAt };
